...
【白名單（只能使用以下名稱；不足就少換點，但不得編造新名稱）】
- 景點清單：$scenicLine
- 美食清單：$foodLine

【硬性規則（務必同時滿足）】
- 每一天的時間軸必須「完整覆蓋」08:00 ～ 20:00，首段從 08:00 開始、末段於 20:00 結束。
- 各段時間需「無縫相接」，不允許任何缺空或重疊：下一段的開始時間 = 前一段的結束時間。
- 每天至少包含「午餐」與「晚餐」各一段，餐廳名稱必須出自美食白名單，並在名稱後加上「（美食）」。
  - 午餐請安排在 11:00–14:00 之間；晚餐請安排在 17:00–20:00 之間。
- 嚴格只從白名單挑選景點/餐廳；若白名單數量不足以填滿 08:00–20:00，請「延長已選項目的停留時間」來填滿，而非新增白名單外名稱。
- 同一天不可出現「非連續重複」的同名景點；若需要延長停留，請以「單一連續時段」呈現。
- 動線需合理、避免遠距離來回；優先符合「$typesLine」、心情/需求與預算（$budgetGuide）。
...
（之後依此格式繼續到 Day $dayCount，並確保當日最後一段結束於 20:00）
— 規格：
1) 全程使用 24 小時制，時間粒度以 15 分鐘為單位（例如 09:00、09:15、09:30、09:45）。
2) 每個區塊必須「連續兩行」：第 1 行為時間區間；第 2 行以「景點：」開頭且名稱必須出自白名單。
3) 兩個區塊之間僅允許「1 個空行」。不得出現其他任何敘述、符號或清單。
